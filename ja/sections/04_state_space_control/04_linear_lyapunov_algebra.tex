\subsection{線形系のリアプノフ安定性}

固有値や伝達関数の極による判定は線形時不変系に有効であるが、同じ安定性を二次形式などのスカラー関数の減少としても表現できる。ここでは内部安定性、すなわち $\dot{x}=Ax$ の原点の漸近安定性を、Lyapunov の直接法で判定する。平衡点 $x=0$ に対し、正定値関数 $V(x)$ があり
\begin{equation}
  V(x)>0\quad(x\ne0),\qquad V(0)=0
\end{equation}
を満たすとする。さらに軌道に沿った微分
\begin{equation}
  \dot{V}(x)=\frac{\pp V}{\pp x}f(x)
\end{equation}
が負定値なら、平衡点は漸近安定である。

線形系 $\dot{x}=Ax$ では、二次形式
\begin{equation}
  V(x)=x^\T P x,\qquad P=P^\T>0
\end{equation}
を考える。微分は
\begin{equation}
  \dot{V}=x^\T(A^\T P+PA)x
\end{equation}
である。任意の正定値 $Q$ に対して
\begin{equation}
  A^\T P+PA=-Q
\end{equation}
を満たす $P>0$ が存在すれば、$A$ は安定である。このリアプノフ方程式は、次に扱う LQR の価値関数へそのままつながる。

正定値行列は、ベクトルの重み付き大きさを作る行列である。$P>0$ なら $x^\T P x$ は $x\ne0$ で常に正になる。半正定値 $P\ge0$ では 0 になる方向が残る。評価関数や共分散行列で正定値性が重要になるのは、距離や不確かさを破綻なく測るためである。

\begin{figure}[!t]
\centering
\includegraphics[width=0.94\linewidth,bb=0 0 1296 1152]{figures/state_space_geometry_examples.png}
\caption{二状態モデル $A=\left[\begin{smallmatrix}0&1\\-2&-3\end{smallmatrix}\right]$、$b=\left[\begin{smallmatrix}0&1\end{smallmatrix}\right]^\T$、$C=\left[\begin{smallmatrix}1&0\end{smallmatrix}\right]$ に対する状態空間の幾何。左上は $K=\left[\begin{smallmatrix}18&6\end{smallmatrix}\right]$ による閉ループ軌道、右上は可制御方向 $B,AB$ と可観測方向 $C^\T,A^\T C^\T$、左下は $L=\left[\begin{smallmatrix}10&10\end{smallmatrix}\right]^\T$ による推定誤差軌道、右下は $A-bK$ に対する $V=x^\T Px$ のレベル集合を示す。}
\label{fig:state-space-geometry-examples}
\end{figure}

図\ref{fig:state-space-geometry-examples} の右下では、閉ループ軌道が楕円の外側から内側へ横切っている。これは $\dot{V}=-x^\T Qx<0$ が、各軌道に沿って $V$ の値を単調に減らすことを幾何的に表している。左上の閉ループ軌道と左下の推定誤差軌道を比べると、制御極とオブザーバ極を別々に配置しても、どちらも原点へ向かう二次元の流れとして確認できる。

図\ref{fig:state-space-geometry-examples} の二状態モデルでは、入力行列を $B=b=\begin{bmatrix}0&1\end{bmatrix}^\T$ とおくと
\begin{equation}
  AB=
  \begin{bmatrix}0&1\\-2&-3\end{bmatrix}
  \begin{bmatrix}0\\1\end{bmatrix}
  =
  \begin{bmatrix}1\\-3\end{bmatrix}
\end{equation}
である。したがって可制御行列は
\begin{equation}
  \mathcal{C}=\begin{bmatrix}B&AB\end{bmatrix}
  =
  \begin{bmatrix}0&1\\1&-3\end{bmatrix},\qquad
  \det\mathcal{C}=-1
\end{equation}
となり、$\rank\mathcal{C}=2$ である。右上に描いた $B$ と $AB$ は、入力から直接動く方向と、その方向が $A$ によって回り込んだ方向を表しており、二つが平面全体を張るので任意の二次元状態を閉ループ設計の対象にできる。同じモデルで
\begin{equation}
  CA=
  \begin{bmatrix}1&0\end{bmatrix}
  \begin{bmatrix}0&1\\-2&-3\end{bmatrix}
  =
  \begin{bmatrix}0&1\end{bmatrix}
\end{equation}
だから、可観測行列は
\begin{equation}
  \mathcal{O}=\begin{bmatrix}C\\CA\end{bmatrix}
  =
  \begin{bmatrix}1&0\\0&1\end{bmatrix},\qquad
  \rank\mathcal{O}=2
\end{equation}
である。出力 $y=Cx$ は第一状態を直接測り、時間変化を通じて第二状態も $CA$ の方向として現れるため、推定誤差の二次元成分をどちらもオブザーバで減衰させられる。

このランク計算は、図\ref{fig:state-feedback-block} と図\ref{fig:observer-separation-block} の信号経路と同じ内容を式で読んだものである。状態フィードバック図では $u=-Kx+r_u$ であり、$r_u=0$ とすれば $u=-Kx$、対象は
\begin{equation}
  \dot{x}=Ax+Bu=(A-BK)x,\qquad y=Cx
\end{equation}
となる。オブザーバ分離図では $u=-K\hat{x}+r_u$、$\hat{y}=C\hat{x}$、$e_y=y-\hat{y}$ であり、推定誤差を $e_x=x-\hat{x}$ とおくと
\begin{align}
  \dot{e}_x
  &=\dot{x}-\dot{\hat{x}}\\
  &=(Ax+Bu)-\{A\hat{x}+Bu+L(y-\hat{y})\}\\
  &=(A-LC)e_x.
\end{align}
$r_u=0$ の推定状態フィードバックでは、$\hat{x}=x-e_x$ なので
\begin{equation}
  \dot{x}=(A-BK)x+BK e_x
\end{equation}
も得られる。したがって、状態の流れは $A-BK$、推定誤差の流れは $A-LC$ によって分かれ、図の軌道と楕円レベル集合は、可制御・可観測な二次元幾何の上で閉ループ安定性を評価している。Lyapunov 関数 $V=x^\T Px$ の楕円が軌道に沿って小さくなるという見方は、後の LQR で二次形式の価値関数を最小化する見方と同じ幾何を使っている。

\subsubsection{離散時間リアプノフ方程式による数値検算}

サンプル値系では、安定性を微分ではなく一ステップ差分で読む。推定誤差の更新を
\begin{equation}
  e_{k+1}=F e_k
\end{equation}
とし、二次形式
\begin{equation}
  V_k=e_k^\T P e_k,\qquad P=P^\T>0
\end{equation}
を用いると、
\begin{align}
  V_{k+1}-V_k
  &=e_{k+1}^\T P e_{k+1}-e_k^\T P e_k\\
  &=(F e_k)^\T P(F e_k)-e_k^\T P e_k\\
  &=e_k^\T(F^\T P F-P)e_k
\end{align}
である。したがって、任意の $Q>0$ に対して
\begin{equation}
  F^\T P F-P=-Q
\end{equation}
を満たす $P>0$ があれば、
\begin{equation}
  V_{k+1}-V_k=-e_k^\T Qe_k<0\qquad(e_k\ne0)
\end{equation}
となり、離散時間でも楕円レベル集合が一ステップごとに内側へ移る。

前節の離散時間オブザーバで得た
\begin{equation}
  F=A_d-L_d C_d\simeq
  \begin{bmatrix}
    0.31277&0.08611\\
    -0.50387&0.73262
  \end{bmatrix}
\end{equation}
に対して $Q=I$ とおく。離散時間リアプノフ方程式を解くと
\begin{equation}
  P\simeq
  \begin{bmatrix}
    1.95004&-0.82627\\
    -0.82627&1.96478
  \end{bmatrix}
\end{equation}
であり、$P$ の固有値は約 $1.13111,2.78371$ なので $P>0$ である。初期推定誤差を
$e_0=\begin{bmatrix}0.20&-0.10\end{bmatrix}^\T$ とすると、
\begin{equation}
  F e_0\simeq
  \begin{bmatrix}0.05394\\-0.17404\end{bmatrix}
\end{equation}
で、次の数値になる。
\begin{equation}
  \begin{array}{c|c}
    \text{量} & \text{値}\\\hline
    V_0=e_0^\T P e_0 & 0.13070\\
    V_1=(F e_0)^\T P(F e_0) & 0.08070\\
    V_1-V_0 & -0.05000\\
    -e_0^\T e_0 & -0.05000
  \end{array}
\end{equation}
この表は、$F^\T P F-P=-I$ という行列等式が、実際の一ステップ更新で
$V_1-V_0=-e_0^\T e_0$ という減少量として現れることを表している。連続時間で
$\dot{V}<0$ と読んだ楕円の内向きの流れは、離散時間では「各サンプルで $V_k$ が減る」
という計算に置き換わる。

\subsection{状態空間の式変形を省略しない}

\begin{theorem}[行列指数関数による解公式]
線形時不変系 $\dot{x}=Ax+Bu$ の解は
\begin{equation}
  x(t)=e^{At}x(0)+\int_0^t e^{A(t-\tau)}Bu(\tau)\,\dd\tau
\end{equation}
である。
\end{theorem}

\begin{derivation}
まず $u=0$ のとき、$x(t)=e^{At}x(0)$ とおく。行列指数関数の微分公式
\begin{equation}
  \frac{\dd}{\dd t}e^{At}=Ae^{At}
\end{equation}
より
\begin{equation}
  \dot{x}(t)=Ae^{At}x(0)=Ax(t)
\end{equation}
である。次に入力がある場合、定数変化法として $x(t)=e^{At}z(t)$ とおく。微分すると
\begin{align}
  \dot{x}(t)&=Ae^{At}z(t)+e^{At}\dot{z}(t),\\
  Ax(t)+Bu(t)&=Ae^{At}z(t)+Bu(t).
\end{align}
両者を等しいとおくと
\begin{equation}
  e^{At}\dot{z}(t)=Bu(t)
\end{equation}
である。左から $e^{-At}$ を掛けて
\begin{equation}
  \dot{z}(t)=e^{-At}Bu(t)
\end{equation}
となる。時刻 $0$ から $t$ まで積分すると
\begin{equation}
  z(t)=z(0)+\int_0^t e^{-A\tau}Bu(\tau)\,\dd\tau.
\end{equation}
$x(0)=z(0)$ なので
\begin{align}
  x(t)&=e^{At}x(0)+e^{At}\int_0^t e^{-A\tau}Bu(\tau)\,\dd\tau\\
  &=e^{At}x(0)+\int_0^t e^{A(t-\tau)}Bu(\tau)\,\dd\tau.
\end{align}
\end{derivation}

\begin{theorem}[Lyapunov の直接法]
平衡点 $x=0$ の近くで、正定値関数 $V(x)$ が存在し、軌道に沿った微分 $\dot{V}(x)$ が負定値なら、その平衡点は漸近安定である。
\end{theorem}

線形系で $V=x^\T Px$ と置くとき、$P=P^\T>0$ なら $V$ は正定値である。微分は積の微分公式より
\begin{align}
  \dot{V}
  &=\dot{x}^\T Px+x^\T P\dot{x}\\
  &=(Ax)^\T Px+x^\T P(Ax)\\
  &=x^\T A^\T Px+x^\T PAx\\
  &=x^\T(A^\T P+PA)x.
\end{align}
ここで $A^\T P+PA=-Q$、$Q>0$ なら
\begin{equation}
  \dot{V}=-x^\T Qx<0\quad(x\ne0)
\end{equation}
なので、Lyapunov の直接法より原点は漸近安定である。
