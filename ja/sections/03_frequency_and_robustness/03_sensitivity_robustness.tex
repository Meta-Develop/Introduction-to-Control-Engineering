...
\wfig{frequency-design-examples} は、安定余裕と感度の読み取りを一つの連続した設計判定として示している。上左の $L_K(s)=K/(s-1)$ では開ループが右半平面極を一つ持つので $P=1$ である。$K=0.6$ ではナイキスト線図が点 $-1$ を囲まず、閉ループ極は $s=1-K=0.4$ となって $Z=1$ の不安定閉ループを与える。$K=1$ では軌跡が点 $-1$ を通り、閉ループ極が原点に来るため安定限界である。$K=2$ では点 $-1$ を 1 回囲み、閉ループ極は $s=-1$ となって $Z=0$ の安定閉ループになる。上右の根軌跡では、三次対象の閉ループ特性多項式が
\begin{equation}
  s(s+2)(s+4)+K=s^3+6s^2+8s+K
\end{equation}
であり、Routh 表の第一列は
\begin{equation}
  1,\qquad 6,\qquad \frac{48-K}{6},\qquad K
\end{equation}
となる。したがって安定条件は $0<K<48$ であり、$K=48$ で第一列の要素が 0 になって根軌跡が虚軸を横切る。下段の $K=0.3,1,3$ の感度比較は同じ制約を性能面から示す。$K=0.3$ は低周波の $\abs{S}$ が最も大きく外乱抑制は弱いが、感度ピークと $\abs{T}$ の高周波側の広がりは小さい。$K=1$ はその中間であり、$K=3$ は低周波の $\abs{S}$ を最も下げる一方、$M_s$ を大きくし、相補感度が通過する帯域を広げる。すなわち、低周波性能を強めるゲイン増加は、ナイキスト線図の $-1$ 近傍への接近、感度ピークの増大、高周波ノイズおよび未モデル化ダイナミクスへの感受性増加として同時に現れる。
