...
\subsubsection{非正弦波信号と帯域制限の対応}

方形波部分和は、非正弦波信号の角や急な変化が高調波で作られることを示した。次に、その高調波が帯域制限をもつ閉ループを通ると時間波形がどう変わるかを、同じ周波数成分のまま確認する。図\ref{fig:frequency-waveform-bridge} では、周期 $T_0=4\,\mathrm{s}$、基本角周波数 $\omega_0=2\pi/T_0$ の方形波指令を考え、閉ループを一次遅れ
\begin{equation}
  T_b(s)=\frac{1}{\tau_c s+1},
  \qquad
  \tau_c=0.35\,\mathrm{s}
\end{equation}
で近似している。これは厳密な制御器設計ではなく、帯域幅が有限であるときに高調波がどのように減るかを見るための最小モデルである。

\begin{figure}[tb]
\centering
\includegraphics[width=0.9\linewidth,bb=0 0 1296 972]{figures/frequency_waveform_bridge.png}
\caption{非正弦波指令を高調波成分へ分け、一次的な閉ループ帯域 $T_b(s)=1/(\tau_c s+1)$ に通したときの対応。上段は時間波形であり、方形波指令の角が帯域制限後に丸められる。左下は各奇数次高調波の入力振幅 $b_k=4/(\pi k)$ と通過後の振幅 $|T_b(jk\omega_0)|b_k$ を比べる。右下は同じ $T_b(j\omega)$ のゲインを Bode 型の減衰曲線として示し、高い次数の成分ほど折点周波数より上で強く減衰することを確認する。}
\label{fig:frequency-waveform-bridge}
\end{figure}

この図で使った入力の周波数成分は、方形波のフーリエ級数により
\begin{equation}
  u(t)=\sum_{\substack{k=1\\k\ \mathrm{odd}}}^{\infty}
  b_k\sin(k\omega_0 t),
  \qquad
  b_k=\frac{4}{\pi k}
\end{equation}
で与えられる。ここまでがフーリエ解析の役割である。フーリエ級数やフーリエ変換は、信号の中にどの周波数成分がどれだけ含まれるかを表すが、その成分が制御対象を通った後にどう遅れ、どれだけ減衰し、過渡成分が安定に消えるかまでは決めない。その情報は、ラプラス変換で得られる伝達関数、極、零点、初期条件を含む過渡応答、安定性の側に属する。

両者が接続されるのは、安定な線形時不変系で過渡成分が減衰した後である。このとき各正弦波成分は周波数を変えずに複素倍率 $T_b(jk\omega_0)$ を受けるので、定常出力は
\begin{equation}
  y_{\mathrm{ss}}(t)
  =
  \sum_{\substack{k=1\\k\ \mathrm{odd}}}^{\infty}
  b_k\abs{T_b(jk\omega_0)}
  \sin\{k\omega_0t+\angle T_b(jk\omega_0)\}
\end{equation}
となる。したがって、図\ref{fig:frequency-waveform-bridge} の丸い立上りは、時間領域で別に仮定した形ではなく、フーリエ成分 $b_k$ と伝達関数の虚軸上の値 $T_b(jk\omega_0)$ を掛け合わせた結果である。Bode 線図は、特定の信号に含まれる周波数成分そのものではなく、安定な線形時不変系が各角周波数の正弦波成分へ与える複素倍率を、周波数に沿って並べた図として読む。
