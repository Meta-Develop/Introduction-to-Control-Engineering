...
モータで軸の角度を目標位置へ合わせる場合、設計者が最初に評価する量は目標値 $r(t)$ と測定値 $y(t)$ の差である。偏差を
\begin{equation}
  e(t)=r(t)-y(t)
\end{equation}
...
\begin{equation}
  u(t)=K_Pe(t)+K_I\int_0^t e(\tau)\,\dd\tau+K_D\dot{e}(t)
\end{equation}
へ進む。ただし積分項は、入力飽和中にも偏差を蓄積するとワインドアップを起こし、飽和が解けた後の大きな行き過ぎを招く。

したがって、リレー、P、PD、PI、PID は経験的な調整手順の名称ではなく、時間応答、安定性、入力飽和、外乱抑制、ノイズ感度を同時に評価する必要性を表す設計上の出発点である。比例ゲインを上げると低周波の偏差は小さくなりやすいが、位相余裕や入力余裕は減ることがある。この相反関係を定量化するため、以後は時間領域の過渡応答、Routh-Hurwitz 判別、根軌跡、Bode 線図、感度関数を順に用いる。
