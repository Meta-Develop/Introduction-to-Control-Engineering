...
本章では、周波数領域へ進む前に、実機の調整で最初に必要になる古典制御の橋渡しを時間領域から整理する。具体的には、ステップ応答の速さと振動、閉ループの信号経路、PID の実装、飽和とアンチワインドアップ、目標値整形、フィードフォワード、同定、残留誤差、性能指標を、測定波形と入力制約に結び付けて扱う。

前章までで、一次遅れ、二次遅れ、ラプラス変換、伝達関数の土台を導入した。ここでは古典制御を、仕様、安定性、補償器構成、実装上の制約を結び付ける設計手順として位置づける。次の周波数領域の章では、本章で見た振動、ロバスト性、帯域幅を、正弦波分解、Fourier 展開・Fourier 変換、Laplace 変換、Bode 線図、Nyquist 判別、根軌跡によってあらためて解釈する。

古典制御で反復して評価する設計基準は四つである。第一に閉ループの漸近安定性、第二に目標値応答の速さと振動の小ささ、第三に外乱およびモデル誤差に対するロバスト性、第四にアクチュエータが要求入力を実現できることである。これらを時間応答、閉ループ信号経路、補償器実装、入力制約、性能指標で相補的に評価し、周波数領域の図法は次章で同じ現象を別の座標から読み直すために使う。
