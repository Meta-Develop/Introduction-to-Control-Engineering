...
一定速度で回るモータ、一定温度に保つ熱系、一定水位に保つタンクのように、制御対象は選んだ定常条件の近くで運転されることが多い。この場合、設計で必要になるのは絶対値そのものだけではなく、測定値が目標とする定常条件からどれだけ外れ、その小さな偏差に入力変更がどの程度効くかである。非線形モデルを全領域で扱う代わりに、まず基準となる動作点を定め、その点からの偏差を変数に取り直し、動作点近傍で有効な局所モデルを構成する。

この基準となる定常条件を形式化するため、次の平衡点を用いる。
...
同じヤコビアン計算を、測定しやすい水槽の水位で確認する。断面積を $A_t$、水位を $h>0$、流入量を $u=q_{\mathrm{in}}$、流出係数を $k_o$ とし、
\begin{equation}
  \dot{h}=f(h,u)=\frac{1}{A_t}\{u-k_o\sqrt{h}\}
\end{equation}
と書く。一定水位 $h_e$ を保つ流入量は
\begin{equation}
  u_e=k_o\sqrt{h_e}
\end{equation}
である。偏差 $\tilde{h}=h-h_e$、$\tilde{u}=u-u_e$ に対する一次モデルの係数は
\begin{align}
  A&=\left.\frac{\pp f}{\pp h}\right|_{(h_e,u_e)}
    =-\frac{k_o}{2A_t\sqrt{h_e}},\\
  B&=\left.\frac{\pp f}{\pp u}\right|_{(h_e,u_e)}
    =\frac{1}{A_t}
\end{align}
である。たとえば $A_t=0.50\,\mathrm{m^2}$、$h_e=0.64\,\mathrm{m}$、$k_o=0.10\,\mathrm{m^{5/2}/s}$ なら $A=-0.125\,\mathrm{s^{-1}}$、$B=2.0\,\mathrm{m^{-2}}$ であり、水位偏差は時定数 $8.0\,\mathrm{s}$ で減る局所モデルになる。同じタンクでも $h_e$ を変えると $\sqrt{h_e}$ が変わるため、線形化係数と時定数も変わる。この計算は、ヤコビアンが「非線形式を平衡点近くでどの傾きとして見るか」を数値で固定する操作であることを示している。
