...
前節のライン追従ロボットでは、時刻をそろえて目標値、センサ測定値、誤差、モータへの入力を並べることで、何を目指し、何を測り、どれだけずれ、そのずれに対して何を加えたかを時系列として解釈できるようになった。次に現れる限界は、測定された出力だけを見ても、次の動き、温度、水位をいつも予測できるわけではないという点である。そこで本節では、信号を入力、出力、外乱、ノイズ、状態という役割に分け、測定値として見えている量と、内部に残って未来を決める情報を区別する。

ライン追従の記録表には、経過時刻、左右センサの明るさ、横ずれ、左右輪のパルス差、PWM 指令のように、同じ時刻に並んだ数列が多く現れる。しかし数列そのものには、制御対象を直接動かす量なのか、センサで測った結果なのか、制御器が次の判断に使う量なのかは書かれていない。同じ $0.12$ という値でも、秒、センサの正規化強度、横方向誤差 m、PWM デューティ比では意味がまったく異なる。したがって、定義名を与える前に、その数列がループのどこへ入り、ロボット本体、測定値、制御器の判断のどれを変えるのかを対応させておく必要がある。表\ref{tab:line-trace-signal-roles} は、前節で観察したライン追従の記録列を、制御系の役割へ写したものである。

\begingroup
\small
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.19\linewidth}>{\raggedright\arraybackslash}p{0.25\linewidth}>{\raggedright\arraybackslash}p{0.24\linewidth}>{\raggedright\arraybackslash}p{0.24\linewidth}@{}}
\caption{ライン追従ロボットの記録列と制御系での役割。数列を、ループへ入る位置と変える対象で分類している。}
\label{tab:line-trace-signal-roles}\\
\hline
記録列 & 具体例と単位 & ループ内での入り方 & 変えるもの\\
\hline
経過時刻 & $t$ [s]、サンプリング時刻 $k\Delta t$ & すべての列を同じ時間軸にそろえる基準 & 変化率、遅れ、履歴の解釈\\
左右センサ強度 & $I_L,I_R$、正規化反射強度 & ライン位置を表すセンサ測定値 $y_m$ の材料 & 測定値と誤差計算\\
横方向誤差 & $e_y$ [m] またはセンサ列上の偏り & 目標線中心からのずれを表す評価量 & 制御量、状態候補、次の指令計算\\
向き誤差・輪パルス差 & $\theta_e$ [rad]、$\Delta N_R-\Delta N_L$ [pulse] & 車体の向き変化を表す測定または推定量 & 状態候補と旋回判断\\
モータ・PWM 指令 & 左右 duty 比 $u_{cL},u_{cR}$ & 制御器がアクチュエータへ渡す指令 & 車輪トルク、速度、実入力\\
出力・制御量 & ロボット中心の横位置、線からのずれ & 目標と比べて小さくしたい結果 & 閉ループ性能の評価\\
外乱 & 床反射率のむら、手で押す作用、段差 & 制御器が選ばず対象やセンサ条件へ入る作用 & 実際の運動またはセンサ強度\\
ノイズ & センサのちらつき、量子化、照明の高速変動 & 真の線位置とは別に測定値へ混じる成分 & 制御器へ届く $y_m$ の揺れ\\
状態候補 & $e_y,\theta_e$、速度 $v$、角速度 $\omega$ & 現在値と入力から次の運動を予測する内部情報 & 未来の横ずれと向き\\
\hline
\end{longtable}
\endgroup

ライン追従ロボットによって、測定列を信号の役割へ分ける方法を整理した。次に熱系へ例を移すのは、同じ出力を測っていても内部に蓄えられた熱のような見えにくい情報が次の変化を決めるため、状態という考え方が必要になることを最も単純に示せるからである。
...
\subsubsection{信号分類と局所近似の数値検証}

図\ref{fig:signal-modeling-paths} では、アクチュエータから対象へ入る実入力 $u$ と外乱 $d$ が対象ブロックの手前で合流する。これらは熱収支の右辺へ入り、温度状態 $T$ の時間変化率を直接変える。一方、測定ノイズ $n$ はセンサ後段で加わるため、同じ温度状態でも制御器へ届く $y_m$ だけを変える。したがって、外乱を状態方程式の入力側に置き、ノイズを測定方程式に置くという分類が、図の信号経路と一致している。

図\ref{fig:signal-modeling-linearization} の値
$T_a=20\,^\circ\mathrm{C}$、$R_{\mathrm{th}}=2.0\,\mathrm{K/W}$、
$C_{\mathrm{th}}=10\,\mathrm{J/K}$、$\eta=0.5\,\mathrm{W/V^2}$、
$v_e=2.0\,\mathrm{V}$ では、平衡温度は
\begin{equation}
  T_e=T_a+R_{\mathrm{th}}\eta v_e^2
  =20+2.0\cdot0.5\cdot2.0^2
  =24\,^\circ\mathrm{C}
\end{equation}
である。ヒータ入力寄与だけを取り出すと、非線形項と一次近似は表\ref{tab:signal-linearization-check} のように比べられる。

\begin{table}[tb]
\centering
\caption{ヒータ電圧偏差に対する非線形入力寄与と一次近似の比較。完全な温度偏差モデルでは、この入力寄与に冷却項 $-\tilde{T}/(R_{\mathrm{th}}C_{\mathrm{th}})$ も加わる。}
\label{tab:signal-linearization-check}
\begin{tabular}{ccc}
\hline
$\tilde{v}$ [V] & 非線形入力寄与 [K/s] & 一次近似 [K/s]\\
\hline
$0.4$ & $0.088$ & $0.080$\\
$1.2$ & $0.312$ & $0.240$\\
\hline
...
$\tilde{v}=0.4\,\mathrm{V}$ では二つの値の差は $0.008\,\mathrm{K/s}$ と小さいが、$\tilde{v}=1.2\,\mathrm{V}$ では差が $0.072\,\mathrm{K/s}$ まで広がる。一次近似は動作点の近くで使いやすいが、偏差が大きくなるほど二次項 $\eta\tilde{v}^2/C_{\mathrm{th}}$ を無視した影響が見える。

図\ref{fig:signal-modeling-euler-step} の一次減衰 $\dot{z}=-z/\tau$、$\tau=0.50\,\mathrm{s}$ では、陽的 Euler 法の更新係数は
\begin{equation}
  1-\frac{h}{\tau}
  =
  \begin{cases}
  0.80, & h=0.10\,\mathrm{s},\\
  -0.20, & h=0.60\,\mathrm{s},\\
  -1.40, & h=1.20\,\mathrm{s}
  \end{cases}
\end{equation}
となる。連続時間の系は常に減衰するが、$h=0.60\,\mathrm{s}$ では符号を交互に変えながら小さくなり、$h=1.20\,\mathrm{s}$ では更新係数の絶対値が 1 を超えて離散計算上の振幅が増える。これは物理対象が不安定になったのではなく、刻み幅が粗すぎるために数値更新が連続時間の安定な減衰を再現できなくなった例である。
