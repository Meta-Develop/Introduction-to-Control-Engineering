...
上段の $R_u=0.03$ の応答は、位置偏差が約 $1.5\,\mathrm{s}$ で零近傍へ入り、$R_u=2$ の応答より明らかに速く整定する。その代わり、入力指令の最大絶対値は初期におよそ $4.8$ となり、入力限界 $\pm 2$ に対する余裕 $2-\max_k |u_c(k)|$ は約 $-2.8$ である。これは限界を越えた指令を前提にした理想線形シミュレーションである。$R_u=2$ では整定は数秒程度まで遅くなるが、最大絶対入力指令はおよそ $0.6$ に抑えられ、入力限界に対する余裕は約 $1.4$ 残る。したがって小さい入力重みは速応性を買う一方で、実機では飽和によって $u_c=-Kx$ がそのまま対象に入らず、実際の入力は $u_a=\operatorname{sat}(u_c)$ になる。飽和区間では設計時に仮定した $x_{k+1}=Ax_k+Bu_c$ と二次コストに基づく最適性が崩れ、閉ループ極、整定時間、推定器へ渡る入力履歴が理想線形 LQR の結果からずれる。

右下の共分散楕円では、$R_v=0.02$ の更新後楕円が位置方向へ強く細くなり、位置測定を大きく信頼していることを示す。$R_v=0.20$ に増やすと更新後楕円は破線の予測楕円に近く残り、測定による縮小量が小さくなる。この変化は
$K=P^-C^\T(CP^-C^\T+R_v)^{-1}$
から直接読める。$R_v$ が大きいほど革新共分散 $CP^-C^\T+R_v$ が大きくなり、Kalman ゲイン $K$ は小さくなる。その結果、更新式 $\hat{x}^+=\hat{x}^-+K(y-C\hat{x}^-)$ で測定残差が状態推定へ反映される量も、$P^+=(I-KC)P^-$ で予測共分散から削られる量も小さくなり、楕円は予測後の不確かさを多く残す。
