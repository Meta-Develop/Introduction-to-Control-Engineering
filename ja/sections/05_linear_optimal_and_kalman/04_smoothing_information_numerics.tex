...
\begin{equation}
  \epsilon_k=\nu_k^\T S_k^{-1}\nu_k
\end{equation}
を監視すると、$Q_w$ と $R_v$ の設定が極端でないかを調べられる。測定次元を $m_y=\dim y_k$ とすると、線形ガウスモデルと共分散が整合している範囲では $\epsilon_k$ はおおよそ $\chi^2_{m_y}$ に従い、短い窓の平均は $m_y$ 付近に集まる。二次元測定なら、次のように革新列を読む。
\begin{center}
\small
\begin{tabular}{@{}p{0.26\linewidth}p{0.34\linewidth}p{0.32\linewidth}@{}}
\hline
$\epsilon_k$ の列の例 & 含意 & 確認する箇所 \\
\hline
$1.6,\,2.4,\,1.9,\,2.2$ & 窓平均が $m_y=2$ 付近で、実測残差と革新共分散が整合している。 & 現在の $Q_w$、$R_v$、モデル構造を基準設定として扱える。 \\
$9,\,11,\,8,\,10$ が続く & 予測した $S_k$ より革新が大きい。$Q_w$ が小さすぎる、$R_v$ が小さすぎる、外乱やモデル構造を取り逃がしている、といった可能性がある。 & 未モデル化外乱を表す $Q_w$、測定を過信していないかを表す $R_v$、$A_k$ と $C_k$ の単位・遅れ・線形化を見直す。 \\
$0.05,\,0.2,\,0.1,\,0.3$ が続く & 予測した $S_k$ が大きすぎる。$Q_w$、$R_v$、または $P_0$ を過大に置いた保守的な推定で、共分散が実際の誤差を覆いすぎている。 & センサ分散、プロセス外乱、状態スケーリングを見直し、共分散整合性を窓平均で再評価する。 \\
$2.1,\,1.8,\,35,\,2.0$ & 単発の外れ値、センサ飛び、スリップ、一時的衝撃が疑われる。全体の $Q_w$ や $R_v$ をただちに変える証拠ではない。 & ゲーティング、ロバスト更新、該当時刻のログを使い、外れ値処理と通常時の共分散調整を分ける。 \\
\hline
\end{tabular}
\end{center}
この表の解釈は、短い数列だけでなく時系列として確認すると実装時の判断に結びつきやすい。図\ref{fig:w05-innovation-trace-examples}は、二重積分器で近似した台車の位置と速度を二次元測定として扱い、同じ Kalman 更新を四つの共分散設定で走らせた例である。上段の細線は各サンプルの $\epsilon_k$、太線は 8 サンプル平均を表す。共分散が整合している場合、平均は $m_y=2$ 付近を上下し、個々のサンプルだけが $\chi^2_2$ の上側確率に従ってときどき大きくなる。ノイズを過小評価し、さらに小さなモデル誤差を含む場合は、太線が継続的に 2 を越え、表の「$9,\,11,\,8,\,10$ が続く」行と同じ診断になる。過度に保守的な共分散では、$\epsilon_k$ が 0 付近に押し込まれ、予測した $S_k$ が実際の革新より大きすぎることを示す。

\begin{figure}[tb]
\centering
\includegraphics[width=0.98\linewidth,bb=0 0 1475 1152]{figures/w05_innovation_trace_examples.png}
\caption{二重積分器で近似した台車の位置・速度測定に対する革新 $\nu_k$ と正規化革新二乗 $\epsilon_k$ の例。上段では細線が各サンプルの $\epsilon_k$、太線が 8 サンプル平均を表し、期待平均 $m_y=2$ と $\chi^2_2$ の 95\% 線を併記している。下段の革新成分を見ると、単発外れ値は孤立した大きな $\nu_k$ と $\epsilon_k$ のスパイクとして現れ、共分散全体をただちに変更すべき継続的な不整合とは区別できる。}
\label{fig:w05-innovation-trace-examples}
\end{figure}

この確認は、Kalman フィルタや LQG を設計した後のログ検証でも同じ意味を持つ。革新は「測定が予測からどれだけ外れたか」であり、$S_k$ はその外れ方をどれだけ許容しているかを表すため、$\epsilon_k$ は両者を同じ無次元尺度で比較する量になる。ただし、入力飽和やレート制限がある閉ループでは、推定器に渡す既知入力と対象へ実際に入った入力が一致していなければ、革新はノイズ共分散ではなく入力不一致を反映して大きくなる。この場合は $Q_w$ や $R_v$ を調整する前に、実入力を推定器へ渡しているか、飽和後の入力で予測しているかを確認する。

したがって、Joseph 形式は半正定値性を保つ代数構造、平方根形式は因子分解で条件数と丸め誤差を抑える構造、情報形式は疎な測定更新と疎 Cholesky 分解を生かす構造を守るための実装技術である。これらは数値計算を壊れにくくするが、$Q_w$、$R_v$、モデル化誤差、外れ値が確率仮定と合っていることまでは保証しない。$\epsilon_k$ の大きさ、継続性、孤立性を読むことが、推定器の共分散が現実の誤差を過小にも過大にも表していないかを調べる確率的なモデル検証になる。
