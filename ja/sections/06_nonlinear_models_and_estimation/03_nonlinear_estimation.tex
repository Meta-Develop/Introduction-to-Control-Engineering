...
\begin{practice}[非線形幾何と推定近似の確認]
図\ref{fig:nonlinear-geometry-estimation-examples}の数値を用いる。まず、$\theta=0.55\,\mathrm{rad}$ で $\abs{\sin\theta-\theta}/\abs{\theta}$ を計算し、橙色の局所線形化領域が小角近似のどの程度の誤差に対応しているかを確認せよ。次に、$V(\pi,0)=2$ を計算し、$c=1.8$ の準位集合が上向き平衡点を含まない理由と、$\dot{V}=0$ の直線全体ではなく最大不変集合を見る必要がある理由を説明せよ。最後に、$\bar{x}=(0.55,0.05)^\T$ から $H=[1.10\ 0.35]$ を導き、EKF の接線更新が粒子雲の曲がった尤度集合を一つの楕円で近似していることを図から読み取れ。
\end{practice}

\subsection{ESKF}

誤差状態 Kalman フィルタ（error-state Kalman filter; ESKF）は、状態そのものを線形化した座標で持つのではなく、非線形な名目状態と、その近くの小さな誤差状態を分けて扱う EKF である。姿勢を四元数 $q$ または回転行列 $R\in\SO(3)$ で表す場合、$q$ には $\norm{q}=1$、$R$ には $R^\T R=I$ と $\det R=1$ という制約がある。通常の EKF のように四元数の 4 成分へ補正量を直接足すと、この制約から外れやすい。ESKF では、姿勢本体は曲面上の名目状態として積分し、共分散は小角度誤差 $\delta\theta\in\R^3$ のような最小座標に持たせる。
...
であり、回転行列で書けば
\begin{equation}
  R^I_B=\hat{R}^I_B\operatorname{Exp}(\widehat{\delta\theta})
  \simeq
  \hat{R}^I_B(I+\widehat{\delta\theta})
\end{equation}
...
\begin{equation}
  ...
  \end{bmatrix}.
\end{equation}
ここで $[r]_\times$ は、$[r]_\times s=r\times s$ を満たす歪対称行列である。第二行の $-\hat{R}^I_B[\hat{a}]_\times\delta\theta$ は、姿勢誤差が比力の慣性系成分を変えることを表す。第三行の $-[\hat{\omega}]_\times\delta\theta-\delta b_g$ は、角速度の名目値まわりで姿勢誤差が回転し、ジャイロバイアス誤差が角度誤差として蓄積することを表す。

\begin{derivation}
速度誤差の主要項は、真の並進式と名目並進式の差から得られる。$a_m-b_a-n_a=\hat{a}-\delta b_a-n_a$ と $R^I_B\simeq\hat{R}^I_B(I+\widehat{\delta\theta})$ を使うと
\begin{align}
  \delta\dot{v}^I
  &=
  ...
  -\hat{R}^I_B\hat{a}\\
  &\simeq
  \hat{R}^I_B\widehat{\delta\theta}\hat{a}
  -\hat{R}^I_B\delta b_a
  -\hat{R}^I_B n_a\\
  &=
...
\begin{practice}[誤差状態と一般化座標の接続]
ESKF の姿勢更新
\begin{equation}
  \hat{q}^{+}=\hat{q}^{-}\otimes\delta q(\widehat{\delta\theta})
\end{equation}
と、単振子で姿勢角を $\theta=\hat{\theta}+\delta\theta$ と分けて考える記述を比較せよ。$\widehat{\delta\theta}$ は更新時に名目姿勢へ注入してリセットする局所誤差であり、$\theta$ は運動エネルギーとポテンシャルエネルギーを記述する一般化座標である。この違いを踏まえ、共分散を持たせる座標、力学式を立てる座標、線形化点として固定する名目値をそれぞれ区別して説明せよ。
\end{practice}
