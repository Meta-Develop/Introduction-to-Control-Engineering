...
線形モデルでこの構造を一度だけ式に戻しておく。状態と測定が
\begin{align}
  x_k&=Ax_{k-1}+Bu_k+w_k,\\
  z_k&=Hx_k+v_k
\end{align}
で表され、$w_k$ と $v_k$ の共分散をそれぞれ $Q$、$R$ とする。このとき、予測は
\begin{align}
...
\begin{align}
  ...
  P_k&=(I-K_kH)P^-_k
\end{align}
で更新する。一次元の相補フィルタとの対応では、$\nu_k$ が加速度計傾きとジャイロ予測の残差、$S_k$ が残差の分散、$K_k$ がその残差を推定値へ戻す比率である。ここで重要なのは、まだ非線形な回転を扱っていない点である。平均を線形モデルで進められるなら、この式だけで推定値と共分散を閉じて更新できる。
...
ここまでで、相補フィルタの固定重みでは扱いにくかったセンサ融合の失敗は、$P^-$、革新、Kalman ゲインという線形 Kalman フィルタの二段構造で読み替えられることが分かる。同時に、ロボットの姿勢予測そのものは $\sin\theta$ と $\cos\theta$ を含むため、固定行列の線形モデルには戻らない。したがって、次の接続は「Kalman フィルタを捨てて別の推定器へ飛ぶ」ことではない。平均姿勢は非線形な運動学 $f$ で進め、誤差と共分散はその時刻の局所ヤコビアン $F_k$ と $G_k$ で進める。さらに位置、方位、ランドマーク、IMU などの測定式も非線形であれば、測定予測 $h(\hat{X})$ のまわりで同じように局所線形化し、革新、残差分散、ゲインの更新式へ入れる。この「非線形な平均」と「線形化した誤差」の分担を明確にしてから、次に扱う拡張 Kalman フィルタへ進む。
