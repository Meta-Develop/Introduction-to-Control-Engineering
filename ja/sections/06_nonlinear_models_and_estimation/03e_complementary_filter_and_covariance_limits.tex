...
この関係は、残差を明示的に書くとさらに見通しがよい。ジャイロで進めた値と加速度計傾きの差
\begin{equation}
  r_k=\theta_{\mathrm{acc},k}-\theta^-_k
\end{equation}
を測定残差と呼ぶと、相補フィルタの更新は
\begin{equation}
  \hat{\theta}_k
  =
  \theta^-_k+(1-\alpha)r_k
\end{equation}
である。先の数値例では $1-\alpha\simeq0.023$ なので、毎サンプルで残差の約 $2.3\,\%$ だけを角度へ戻していることになる。これは静止時、通常走行時、急加速時を同じ比率で扱うという仮定である。これに対して、分散比で決める $K_k$ は表\ref{tab:flow-six-scalar-kalman-gain} のように変わる。

\begin{table}[tb]
  \centering
  \caption{予測不確かさ $P^-_k$ と測定分散 $R_{\mathrm{acc},k}$ による一次元 Kalman ゲインの変化。数値は角度推定の説明用で、単位はいずれも $\mathrm{rad}^2$ である。}
  \label{tab:flow-six-scalar-kalman-gain}
  \begin{tabular}{c|c|c|c|p{0.38\linewidth}}
    状況 & $P^-_k$ & $R_{\mathrm{acc},k}$ & $K_k$ & 解釈\\
    \hline
    静止に近い & $4.0\times10^{-4}$ & $1.0\times10^{-4}$ & $0.80$ & 加速度計傾きが細く、予測の広がりを大きく戻す。\\
    通常走行 & $1.0\times10^{-4}$ & $4.0\times10^{-4}$ & $0.20$ & ジャイロ予測を主に使い、測定でゆっくり補正する。\\
    急加速 & $1.0\times10^{-4}$ & $2.5\times10^{-3}$ & $0.038$ & 加速度計の見かけの傾きが粗いため、測定残差を強く抑える。\\
    ドリフト蓄積後 & $1.6\times10^{-3}$ & $4.0\times10^{-4}$ & $0.80$ & 予測側の不確かさが増え、同じ測定でも補正を大きくする。\\
  \end{tabular}
\end{table}

表の通常走行では $K_k=0.20$ であり、固定相補フィルタの $0.023$ より測定をかなり強く使う。急加速では $R_{\mathrm{acc},k}$ を大きく見積もるため $K_k=0.038$ まで下がり、残差が大きくても角度へほとんど入れない。反対に、ジャイロバイアスの蓄積などで $P^-_k$ が大きくなった後は、同じ $R_{\mathrm{acc},k}$ でも $K_k=0.80$ になり、予測を測定側へ強く戻す。この数値比較は、Kalman ゲインが単なる調整つまみではなく、予測と測定のどちらをどれだけ疑うかを分散で計算した係数であることを示している。更新後の分散も
\begin{equation}
  P_k=(1-K_k)P^-_k
\end{equation}
となり、強く測定で補正したときほど角度推定の不確かさは小さくなる。
