...
図\ref{fig:flow-six-rigid-body-force-moment-geometry} は、この入力写像を 6 自由度状態方程式へ渡すときの幾何と量の対応関係をまとめている。体軸成分の力 $f^B$ は $R^I_B f^B$ として慣性座標系の並進式へ渡し、作用点の腕 $r^B$ は $r^B\times f^B$ の向きで重心まわりモーメントを作る。回転式では、現在の角速度 $\omega^B$ と外モーメント $\tau^B$ を同じ物体固定座標系でそろえてから、慣性テンソル $J^B$ を通して角加速度 $\dot{\omega}^B$ を求める。慣性主軸と物体固定軸が一致していない場合は慣性乗積が残り、モーメントと角加速度が同じ向きになるとは限らない。したがって、図の下段のように、量、座標系、使用先を並べて管理することが、数値実装の検算になる。

\begin{figure}[tb]
  \centering
  \includegraphics[width=0.98\linewidth]{figures/flow6_rigid_body_force_moment_geometry.png}
  \caption{6 自由度剛体モデルにおける力、モーメント、慣性写像の幾何。体軸成分の力は $R^I_B f^B$ により慣性座標系へ写され、重心からの腕は $r^B\times f^B$ の符号付きモーメントを作り、回転式では $J^B$、$\omega^B$、$\dot{\omega}^B$ を物体固定座標系でそろえて扱う。実装で $R^I_B$、$r^B$、$J^B$ のいずれかを落とすと、座標変換、モーメント生成、慣性写像のどれかが欠けるため、実装されたプラントそのものが変わる。}
  \label{fig:flow-six-rigid-body-force-moment-geometry}
\end{figure}
...
図\ref{fig:flow-six-rigid-body-force-moment-geometry} の下段右に示した数値を、状態方程式へ入る順番に沿って検算する。質量 $m=2$ [kg] の剛体を考える。姿勢は水平面内のヨー角 $\psi=\pi/6$ [rad] だけ回転しており、$R^I_B=R_z(\psi)$ とする。物体固定座標系で、重心から
\begin{equation}
  r^B=
