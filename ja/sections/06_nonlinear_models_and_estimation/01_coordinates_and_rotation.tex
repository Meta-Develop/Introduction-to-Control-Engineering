...
\begin{equation}
  ...
  \end{bmatrix}
\end{equation}
となり、姿勢は $\phi-\psi$ だけに依存する。したがって、$\phi$ と $\psi$ を別々に変えても同じ姿勢を表す組が存在する。この近くでは、わずかな角速度をオイラー角速度へ換算するだけで非常に大きな $\dot{\phi}$ や $\dot{\psi}$ が計算されることがあり、数値積分、EKF のヤコビアン、姿勢制御の誤差計算を不安定にする。
...
\end{equation}
である。第二式は、$\{A\}$ から $\{B\}$ 原点まで進み、さらに $\{B\}$ 成分で与えられた $\{C\}$ 原点の位置を $\{A\}$ 成分へ変換して加える、という合成規則を表している。
...
\end{equation}
のように、位置誤差と小角度姿勢誤差を推定座標系側のベクトルとして取り出せる。ここで $\operatorname{Log}$ は後で形式的に定義する回転群の対数写像であり、$e_p$ の単位は [m]、$e_R$ の単位は [rad] である。この表現は、姿勢制御、ESKF、NMPC で、名目 pose は $\operatorname{SE}(3)$ 上に保ち、誤差だけを局所的なベクトルとして線形化するための基礎になる。
...
$R^I_B$ は $3\times3$ 行列なので 9 個の成分を持つが、$R^\T R=I$ と $\det R=1$ の制約により独立な姿勢自由度は 3 である。したがって、数値積分や EKF で行列成分に任意の補正を足すと、一般には $\SO(3)$ から外れる。姿勢を状態に含む制御と推定では、回転そのものは $\SO(3)$ 上に保ち、小さな誤差だけを三次元ベクトルとして扱う設計が必要になる。この事実が、四元数、指数写像、誤差状態フィルタへつながる。
...
四元数は特異点を避ける実用的な姿勢表現であるが、姿勢誤差を最小の 3 成分で扱うには $\SO(3)$ の接空間を使うと整理しやすい。ベクトル $\varphi\in\R^3$ から歪対称行列を作る写像を
\begin{equation}
...
\end{equation}
である。$\theta$ が 0 に近いときは数値的に $0/0$ に近づくため、級数近似を使う。$\theta$ が $\pi$ に近いときは回転軸の符号選択が不安定になりやすい。したがって、対数写像は局所誤差には非常に有用だが、$180^\circ$ 近傍の大誤差を単純な線形誤差として扱うことは避ける。
...
\end{equation}
のように表す。右辺の $\delta\theta$ は、$\hat{R}$ そのものへ直接足す量ではなく、接空間で表した小回転である。誤差行列
\begin{equation}
  \tilde{R}=\hat{R}^\T R
\end{equation}
を使えば
...
\begin{equation}
  ...
  \end{bmatrix}
\end{equation}
である。これは後で用いる誤差状態カルマンフィルタの更新式の基礎になる。
...
以上の理由から、オイラー角、四元数、指数写像、対数写像は競合する表現ではなく、用途の異なる表現である。オイラー角は表示と仕様の記述に適し、四元数は特異点のない姿勢積分と合成に適し、指数写像と対数写像は小さな誤差、共分散、フィードバック誤差を三次元ベクトルとして扱うために適している。姿勢を含む EKF、ESKF、NMPC、剛体姿勢制御では、これらを区別して使うことが、座標特異点と単位制約を破らない実装につながる。
