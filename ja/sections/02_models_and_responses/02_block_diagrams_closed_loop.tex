...
\begin{tikzpicture}[x=1cm,y=1cm,font=\small]
  \node[align=center] (reference) at (0,0) {$R(s)$};
  \node[control sum] (error) at (1.25,0) {$\begin{array}{c}+\\[-3pt]-\end{array}$};
  \node[control small block] (controller) at (2.65,0) {$C(s)$};
  \node[control sum] (inputsum) at (4.05,0) {$\begin{array}{c}+\\[-3pt]+\end{array}$};
  \node[control small block] (plant) at (5.45,0) {$G(s)$};
  \node[control sum] (outputsum) at (6.85,0) {$\begin{array}{c}+\\[-3pt]+\end{array}$};
  \node[control dot] (takeoff) at (8.05,0) {};
  \node[align=center] (output) at (9.05,0) {$Y(s)$};
  \node[control sum] (noisesum) at (8.05,-1.55) {$\begin{array}{c}+\\[-3pt]+\end{array}$};
  \node[align=center] (inputdist) at (4.05,1.1) {$D(s)$};
  \node[align=center] (outputdist) at (6.85,1.1) {$D_y(s)$};
  \node[align=center] (noise) at (9.05,-1.55) {$N(s)$};

  \draw[control signal] (reference) -- (error);
  \draw[control signal] (error) -- node[control signal label,above] {$E(s)$} (controller);
  \draw[control signal] (controller) -- (inputsum);
  \draw[control disturbance] (inputdist) -- (inputsum);
  \draw[control signal] (inputsum) -- (plant);
  \draw[control signal] (plant) -- (outputsum);
  \draw[control disturbance] (outputdist) -- (outputsum);
  \draw[control signal] (outputsum) -- (takeoff);
  \draw[control signal] (takeoff) -- (output);
  \draw[control signal] (takeoff) -- (noisesum);
  \draw[control disturbance] (noise) -- (noisesum);
  \draw[control signal] (noisesum.west) -- ++(-6.8,0)
    node[control signal label,above,pos=0.48] {$Y_m(s)$} |- (error.south);
\end{tikzpicture}%
}
...
\end{equation}
である。$L=CG$ として、出力を左辺に集めると
\begin{align}
  Y(s)&=L(s)R(s)-L(s)Y(s)-L(s)N(s)+D_y(s),\\
  \{1+L(s)\}Y(s)&=L(s)R(s)+D_y(s)-L(s)N(s).
\end{align}
両辺を $1+L(s)$ で割れば
\begin{align}
  Y(s)
...
\begin{align}
  ...
  =
  \lim_{s\to0}\frac{1}{s+sL(s)}
  =
  \frac{1}{\lim_{s\to0}sL(s)}
  =
  \frac{1}{K_v},
  ...
  =
  \lim_{s\to0}\frac{1}{s^2+s^2L(s)}
  =
  \frac{1}{\lim_{s\to0}s^2L(s)}
  =
  \frac{1}{K_a}.
...
\begin{equation}
  ...
  U(s)=C(s)\{R(s)-Y(s)\}
\end{equation}
である。二つの式を代入し、出力を含む項を左辺に集めると
\begin{align}
  Y(s)
  &=G(s)\left[C(s)\{R(s)-Y(s)\}+D(s)\right]\\
  &=C(s)G(s)R(s)-C(s)G(s)Y(s)+G(s)D(s),\\
  \{1+C(s)G(s)\}Y(s)
  &=C(s)G(s)R(s)+G(s)D(s)
\end{align}
である。$L(s)=C(s)G(s)$ と書けば
\begin{equation}
...
\begin{align}
  ...
  &=
  \frac{\lim_{s\to0}sG(s)}
       {\lim_{s\to0}\left[s+\{K_Ps+K_I\}G(s)\right]}\\
  &=
  \frac{0}{K_I}
  =
  0
