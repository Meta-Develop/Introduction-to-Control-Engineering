...
\begin{equation}
  ...
  \end{bmatrix}
\end{equation}
が最適解である。この例では、等式制約だけの射影解は $[0\ 2]^\T$ であり、総入力は満たすが第二入力の上限を破る。上下限制約を含めた最適解は $[0.5\ 1.5]^\T$ へ移り、能動制約は第二アクチュエータの上限制約 $u_2\le1.5$ である。対応する正の乗数 $\mu=1>0$ は、第二入力をさらに増やしたい要求が上限面で押し返されている大きさ、すなわちアクチュエータ余裕への圧力として読める。制約なしの希望値は第二入力を強く使いたいが、上限がアクティブになるため、残りの総入力を第一入力へ配分している。MPC の入力列でも同じように、目標追従だけで決まる入力が制約を破ると、ソルバはアクティブ制約を満たす面の上で最も評価関数が小さい入力列を選ぶ。

この QP の解法は、以後の MPC、NMPC、制御配分の共通の検査点になる。ログでは、最大制約違反がハード制約からの逸脱量、能動制約集合が現在接触している上限・下限・等式面、KKT 残差が停留条件と相補性の未収束、適用入力または配分入力が \wfig{mpc-architecture} のプラントへ送られた実際の信号を表す。後の \wfig{mpc-constraint-examples} では、この情報を可行集合上の境界と対応させる。
