...
\begin{practice}[KKT 条件と能動制約の確認]
\wfig{mpc-constraint-examples} の左図で、赤い二本の境界を $g_1(z)\le0$、$g_2(z)\le0$ とおく。制約付き最適点 $z^*$ に対して、停留条件
\begin{equation}
  \nabla J(z^*)+\mu_1\nabla g_1(z^*)+\mu_2\nabla g_2(z^*)=0
\end{equation}
と相補性 $\mu_i g_i(z^*)=0$ を書き、赤くない境界に対応する乗数がなぜ 0 になるかを説明せよ。右図では、入力幅を狭くしたときに長く能動になる入力制約を一つ選び、その制約の乗数が大きい場合に、追従性能と制約余裕のどちらが閉ループ応答を支配しているかを述べよ。
\end{practice}
...
\begin{practice}[線形 MPC の QP データ確認]
一状態一入力で $N_p=2$ とし、上の $H$、$g(x_{k|k})$、状態制約行列を使う。$x_{k|k}$ を大きくすると、$H$、$g$、状態制約の右辺のうち、どれが変わるかを分類せよ。次に、\wfig{mpc-constraint-examples} の右図で入力幅 $u_{\max}$ を狭くした場合を想定し、QP ログに残すべき量として、最大制約違反、能動制約、KKT 残差、適用入力 $u^*_{0|k}$ のうち少なくとも三つを選び、それぞれが閉ループのどの異常を見つけるかを説明せよ。
\end{practice}
...
\end{equation}
を一つ前へシフトし、最後に終端制御則、定常入力、または最後の入力保持を付けて、時刻 $k$ の初期軌道 $\bar{z}_k$ を作る。これをウォームスタートという。測定または推定された新しい初期状態 $\hat{x}_k$ を反映し、$\bar{z}_k$ のまわりでモデルと制約を線形化して QP を解く。得られたステップ $\Delta z_k$ で
\begin{equation}
...
\begin{practice}[NMPC の一反復と採用条件の確認]
\wfig{mpc-architecture} の最適化器を NMPC に置き換え、直接多重射撃法を使うとする。時刻 $k$ で、前回軌道をシフトした初期軌道、欠陥制約 $x_{i+1}-\Phi_i(x_i,u_i)=0$、線形化 QP、最初の入力 $u_{0|k}$ の採用条件を、処理順に並べよ。さらに、制限時間で SQP が一回しか進まなかった場合に、採用してよい入力と採用してはならない入力を、ハード制約違反、最大欠陥残差、計算時間、監視フラグの観点から区別せよ。
\end{practice}
...
\begin{practice}[制御配分と飽和診断の確認]
$B=\begin{bmatrix}1&1\end{bmatrix}$、$w=2$ とする。まず制約なしの最小ノルム解 $u=B^+w$ を求めよ。次に、前半の二アクチュエータ QP 例と同じく、第二入力を大きく使いたい希望値 $r=[1\ 3]^\T$ と上限制約 $u_i\le1.5$ がある場合、最適解 $u^*=[0.5\ 1.5]^\T$ でどの制約が能動になるかを答えよ。最後に、実装ログとして $w-Bu^*$、飽和フラグ、該当制約の KKT 乗数を残す理由を、制御器側の性能診断とアクチュエータ側の余裕診断に分けて説明せよ。
\end{practice}

\subsection{実装条件をモデルへ入れる}

\wfig{implementation-effects-examples} で確認した実装差は、最後に付ける補正ではなく、\wfig{mpc-architecture} の対象へ入る入力信号を定義し直す条件である。入力飽和を単なる注意として扱うと、理論と実機が離れる。計算入力 $u_c$ と実入力 $u$ の関係を
\begin{equation}
...
\begin{practice}[実入力モデルと制約余裕の確認]
\wfig{implementation-effects-examples} の赤線では、計算入力 $u_c$ と実入力 $u^{act}$ が一致しない。入力飽和、レート制限、一サンプル遅れのうち二つを選び、予測モデルで使うべき入力が $u_c$ ではなく $u^{act}$ または $u_{k-1}^{act}$ になる理由を式で示せ。さらに、拡大状態 $\xi_k=[x_k^\T\ u_{k-1}^\T]^\T$ を使ったとき、MPC の初期条件として現在状態の推定値のほかに何を保存しておく必要があるかを答えよ。
\end{practice}
