...
\end{picture}
\caption[MPC の閉ループ構造]{MPC の閉ループ構造。参照、推定状態、予測モデル、制約集合から最適化問題を作り、解かれた入力列の先頭だけを対象へ適用し、測定出力で次の時刻に解き直す。}
...
\caption[MPC と QP の制約可視化]{MPC と QP の制約を視覚化した例。左は決定変数 $z=(u_0,u_1)$ の可行集合、二次目的関数の等高線、制約なし最小点、制約付き最適点を示し、赤い境界が能動制約である。右は $x_{k+1}=0.8x_k+0.5u_k$、$N=8$、初期値 $x_0=1.8$ の閉ループで、入力上下限 $[-u_{\max},u_{\max}]$ を変えたときの状態と適用入力を比較している。入力幅が狭いほど負方向の下限 $-u_{\max}$ に張り付き、状態の減衰が遅くなる。}
\label{fig:mpc-constraint-examples}
\end{figure}

\wfig{mpc-constraint-examples} の左図では、制約なし最小点が可行集合の外にあるため、QP は可行多面体の境界上で最適点を選ぶ。赤い二本の境界は入力上限 $u_0\le1$ と、縮約後に $u_0+0.7u_1\le1.25$ の形で現れる結合線形制約を表す。最適点は $z^\star=(1,0.357\ldots)$ であり、これら二つの不等式がともに等号で満たされる。制約なし最小点へ向かう降下方向が二つの境界を外側へ越えようとするため、KKT 条件ではこの二本の外向き法線の非負結合で勾配を打ち消す形になり、対応する乗数は正になり得る。実際、この例の縮約 QP では $u_0\le1$ の乗数が約 $0.743$、$u_0+0.7u_1\le1.25$ の乗数が約 $0.686$ となり、残りの不等式には余裕が残る。

右図の閉ループ計算でも、制約幅は応答を直接変える。$u_{\max}=0.25$ では入力が $k=0,1,2,3,4$ の五サンプルで負側下限に張り付き、状態が $x_k<0.2$ になるのは $k=5$ である。一方、$u_{\max}=1.00$ では飽和は $k=0,1$ の二サンプルだけで、$x_k<0.2$ は $k=3$ に早まる。したがって、MPC の調整では重み $Q,R$ だけでなく、制約幅、制約余裕、ホライズン長、終端条件が同じ閉ループ応答を決める。
