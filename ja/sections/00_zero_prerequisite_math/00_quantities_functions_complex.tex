...
同じ換算を $\xi=1.5$ でも数値まで追う。正規化応答は二つのケースで共通なので、
\begin{equation}
  x=e^{-1.5}\simeq 0.223,\qquad
  t=\xi\tau,\qquad
  \theta=\theta_0 x,\qquad
  \theta_F=\frac{9}{5}\theta
\end{equation}
である。したがって、ケース A とケース B の単位つきの値と無次元比は次の表のように分かれる。
\begin{center}
\small
\begin{tabular}{c|c|c|c|c}
ケース & $t=\xi\tau$ [s] & $\theta=\theta_0 x$ [K] & $\theta_F=\frac{9}{5}\theta$ [$^\circ\mathrm{F}$ の差] & $x=e^{-1.5}$ \\
\hline
A & $90$ & $4.46$ & $8.03$ & $0.223$ \\
B & $180$ & $2.23$ & $4.02$ & $0.223$
\end{tabular}
\end{center}
ケース B はケース A より時定数が 2 倍であるため、同じ $\xi=1.5$ に対応する実時間は $90\,\mathrm{s}$ から $180\,\mathrm{s}$ へ 2 倍になる。初期偏差は半分なので、温度偏差も $4.46\,\mathrm{K}$ から $2.23\,\mathrm{K}$ へ半分になり、華氏温度差も同じ比で $8.03$ から $4.02$ へ変わる。一方で、どちらも $\theta/\theta_0=e^{-1.5}\simeq 0.223$ であり、正規化した量は同じ減衰割合を表している。
